...
\thinkbox{The robot follows the edge of a line (not the line itself). The \code{reflectance_setpoint} is calculated to be grey, the average of the reflectance values measured for the line and road. Suppose the white line is on the left and the dark road background on the right. When the sensor sees more dark road than the setpoint, the robot needs to turn to the left to find the line and the opposite for when it sees too much white line. This is why the robot can only follow one side of the line, rather than the middle.}